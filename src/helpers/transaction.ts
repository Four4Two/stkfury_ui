...
import { decodeTendermintClientStateAny, decodeTendermintConsensusStateAny } from "./utils";
...
export async function MakeIBCTransferMsg({channel, fromAddress, toAddress, amount, timeoutHeight,
                                           timeoutTimestamp = IBCConfiguration.timeoutTimestamp,
                                           denom,
                                           sourceRPCUrl, destinationRPCUrl, port = "transfer"}:any) {
  const tendermintClient = await tendermintRPC.Tendermint34Client.connect(sourceRPCUrl);
  ...
  return await ibcExtension.ibc.channel.clientState(port, channel).then(async (clientStateResponse: QueryChannelClientStateResponse) => {
    ...
    if (destinationRPCUrl === undefined) {
      ...
      const remoteTendermintClient = await tendermintRPC.Tendermint34Client.connect(destinationRPCUrl);
