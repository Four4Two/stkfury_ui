...
const Filters = ({setSortActive, sortActive, searchHandler}:any) => {
    ...
    const buttonClass = 'filterButton rounded-md py-2.5 px-4 ' +
    ...
    return (
            <div className="flex items-center justify-between mb-6 flex-wrap">
                <div>
                    <button className={`${sortActive["all"] ? "active" : ""} `+buttonClass}
                            onClick={() => handleSort("all")}>
                        All
                    </button>
                    <button className={`${sortActive["dexes"] ? "active" : ""} `+buttonClass}
                            onClick={() => handleSort("dexes")}>
                        Decentralized Exchanges
                    </button>
                    <button className={`${sortActive["lending"] ? "active" : ""} `+buttonClass}
                ...
                <div>
                    <div className={`w-[196px] relative my-2`}>
                        ...
                        />
                        <Icon iconName="search" viewClass={`${styles.searchInputIcon} absolute`}/>
                    </div>
                </div>
            </div>
    );
