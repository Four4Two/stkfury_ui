...
import {decimalize, truncateToFixedDecimalPlaces, unDecimalize} from "../../../../helpers/utils";
...
const Submit = () => {
    ...
    const {
        connect, isWalletConnected, persistenceAccountData, persistenceSigner, persistenceChainData,
        cosmosAccountData, cosmosChainData
    } = useWallet()

    const stkAtomAmount = Number(amount) - (Number(amount) * redeemFee)
    const atomAmount = Number(stkAtomAmount) / exchangeRate
    const amountFee: number = truncateToFixedDecimalPlaces(atomAmount);
  ...
  const enable = amount && (Number(amount) > 0) &&
      ...
      <Button
        ...
        content={(name === UN_STAKE && inProgress) ? <Spinner size={"medium"} /> :
            (Number(amount) > Number(stkAtomBalance)) ?
                ...
                :
                    type === INSTANT ?
                        Number(amount) > Number(decimalize(maxRedeem)) ? "Not enough liquidity" : "Redeem Instantly"
                    :
                        "Unstake"
      }
