  ...
  const optionHandler = (value: unStakeType) => {
    ...
    return(
      <div className={`${styles.options} rounded-md flex flex-wrap flex-col p-6 bg-input border border-solid border-[#1b1b1b99] md:p-3`}>
        ...
        <div className={`${type !== INSTANT ? 'active' : ''} option-item cursor-pointer block relative rounded-lg p-6 border border-solid border-[#373737] md:p-4`}
             onClick={() => optionHandler('normal')}>
          <div className="flex items-center">
            <p className="option-heading m-0 font-normal text-base text-light-disabled leading-normal flex-1 md:text-sm">Unstake in 21 days</p>
            <p className={`${styles.amount} option-value font-medium m-0 text-light-mid text-2xl text-center text-right overflow-x-auto md:text-lg`}>{atomAmount} ATOM</p>
          </div>
        </div>
      </div>
